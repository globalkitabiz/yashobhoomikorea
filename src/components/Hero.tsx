...
export default function Hero() {
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden pt-32 sm:pt-28">
      ...
      {/* Lighter overlay — lets photos show through */}
      <div className="absolute inset-0 bg-gradient-to-b from-[var(--navy-dark)]/70 via-black/50 to-[var(--navy-dark)]/70" />

      <div className="relative z-10 max-w-5xl mx-auto px-4 text-center">
        {/* Text area with backdrop blur for readability */}
        <div className="rounded-3xl backdrop-blur-md bg-black/25 px-6 sm:px-10 py-10 sm:py-14 shadow-2xl border border-white/10">
          <div className="inline-flex items-center gap-2 bg-white/15 backdrop-blur-sm border border-white/20 rounded-full px-4 sm:px-5 py-2 mb-8">
            <span className="text-[var(--gold)] text-xs sm:text-sm font-medium drop-shadow-lg">IICC × ZEE TV × AI FUTURE Group</span>
          </div>

          <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold text-white leading-tight mb-4 drop-shadow-[0_2px_8px_rgba(0,0,0,0.6)]">
            <span className="block text-2xl sm:text-3xl md:text-4xl font-light text-white/80 mb-2">2026</span>
            <span className="block text-[var(--gold)] drop-shadow-[0_2px_12px_rgba(200,150,62,0.4)]">{t("K20 India-Korea", "K20 인도-한국")}</span>
          ...
          <p className="text-white/70 text-sm sm:text-base max-w-2xl mx-auto mt-4 mb-2 italic drop-shadow-md">
            {t(SITE_CONFIG.theme, "혁신의 이전: 지속 가능한 한-인도 경제 회랑 구축")}
          </p>
          <p className="text-[var(--gold)] text-xs sm:text-sm font-semibold mb-6 drop-shadow-md">
        ...
        <div className="flex flex-wrap justify-center gap-3 mt-8">
          {[
            ...
            <span key={tag} className="px-4 py-1.5 text-xs font-medium text-white/90 border border-white/20 rounded-full backdrop-blur-md bg-black/30 drop-shadow-md">{tag}</span>
          ))}
        </div>
      </div>
